...
QWQ_MODEL_PATH = "" #学长的模型路径(需用qwq 32b)
DATASET_DIR = os.path.join(BASE_DIR, "mathfusionqa_math")

OUTPUT_DIR = os.path.join(BASE_DIR, "mathfusionqa_math_qwq")
...
def load_all_parquets(dataset_dir):
    ...
    for fname in sorted(os.listdir(dataset_dir)):
        ...
        if fname.endswith(".parquet"):
            ...
            subset_name = fname.split("-00000")[0] 
            for _, row in df.iterrows():
                item = row.to_dict()
                item["_subset"] = subset_name
                all_data.append(item)
    print(f"Successfully loaded {len(all_data)} samples from {dataset_dir}")
    return all_data
